...
import { resolveMassConversion } from './unit-converter.js';
...
export class FormHandler {
    ...
    normalizeUnit(unit) {
      ...
      const unitAliases = {
        ...
        'whole': 'unit', 'slice': 'slice', 'slices': 'slice',
      ...
      const validUnits = ['g', 'kg', 'cup', 'tbsp', 'tsp', 'ounce', 'lb', 'ml', 'l', 'dl', 'unit', 'slice', 'bunch', 'clove'];
    ...
    extractIngredientName(fullName) {
        ...
        const coreIngredients = [
            ...
            'soy sauce', 'oyster sauce', 'sweet pepper',
        ...
        row.innerHTML = `
            ...
            <td data-label="Unit">
                <select class="unit-input" data-index="${index}">
                    ${['g', 'kg', 'ounce', 'lb', 'cup', 'tbsp', 'tsp', 'ml', 'dl', 'l', 'unit', 'slice', 'bunch', 'clove'].map(unit => `
    ...
    getIngredients() {
      return this.selectedIngredients.map(ingredient => ({
        ...ingredient,
        conversion: this.getMassConversion(
          ingredient.amount || 0,
          ingredient.unit,
          ingredient.name || ingredient.matchedTo || ingredient.displayName
        )
      }));
    }
    ...
    getRecipeLabel() {
      const names = this.selectedIngredients
        .map(ingredient => ({
          ...
          amountInTons: this.getMassConversion(
            ingredient.amount || 0,
            ingredient.unit,
            ingredient.name || ingredient.matchedTo || ingredient.displayName
          ).tons ?? 0
    ...
    async calculateImpact() {
      ...
      for (const ing of ingredients) {
          ...
          const conversion = this.getMassConversion(
            ing.amount || 0,
            ing.unit,
            ing.name || ing.matchedTo || ing.displayName
          );
          if (conversion.tons === null) {
            throw new Error(conversion.error);
          }
          const amountInTons = conversion.tons;
    ...
    getMassConversion(amount, unit, ingredientName) {
      return resolveMassConversion({
        amount,
        unit: this.normalizeUnit(unit),
        ingredientName,
        conversionMap: DataManager.conversionMap
      });
    }

    convertToTons(amount, unit, ingredientName = null) {
      return this.getMassConversion(amount, unit, ingredientName).tons;
    }
}
