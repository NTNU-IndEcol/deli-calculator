...
export class ResultsView {
    ...
    async exportAllMetricsData() {
        ...
        const csvRows = [
            ...
            ['recipe_label', 'ingredient', 'category', 'amount', 'unit', 'source', 'matched_ingredient', 'commodity_code', 'mass_g', 'conversion_route', 'factor_g_per_unit', 'conversion_source', 'fallback_used'].join(',')
        ];
        ...
        this.latestIngredients.forEach(ingredient => {
            csvRows.push([
                ...
                ingredient.conversion?.grams ?? '',
                this.escapeCsvValue(ingredient.conversion?.route || ''),
                ingredient.conversion?.factorGramsPerUnit ?? '',
                this.escapeCsvValue(ingredient.conversion?.source || ''),
                ingredient.conversion?.fallback ?? ''
            ].join(','));
        });
