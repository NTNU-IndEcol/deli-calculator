...
import { buildConversionMap } from './unit-converter.js';
...
export class DataManager {
    ...
    static async loadConversionFactors() {
      try {
        ...
        if (!parsed[0]?.Ingredient || !parsed[0]?.Amount || !parsed[0]?.Unit || !parsed[0]?.Grams) {
          throw new Error("Invalid CSV format - missing required columns");
        }
    
        this.conversionMap = buildConversionMap(parsed);

        if (this.conversionMap.size !== parsed.length) {
          console.info(`${parsed.length - this.conversionMap.size} conversion records are retained for provenance but excluded from ingredient-specific lookup`);
        }
