...
# Windows terminals may default to a legacy code page that cannot encode the
# status symbols used by this module. Preserve the messages while replacing
# only unsupported glyphs instead of letting logging abort recipe extraction.
...
class RecipeExtractor:
    ...
    def extract_recipe_from_url(self, url):
        ...
        try:
            ...
            if recipe_data and recipe_data.get('recipeIngredient'):
                ...
                recipe_data['extractionMethod'] = 'json_ld'
                return recipe_data
            ...
            if recipe_data and recipe_data.get('recipeIngredient'):
                ...
                recipe_data['sourceUrl'] = url
                recipe_data['extractedAt'] = time.strftime('%Y-%m-%d')
                recipe_data['extractionMethod'] = 'site_specific'
                return recipe_data
            ...
            if recipe_data and recipe_data.get('recipeIngredient'):
                ...
                recipe_data['sourceUrl'] = url
                recipe_data['extractedAt'] = time.strftime('%Y-%m-%d')
                recipe_data['extractionMethod'] = 'html_meta'
                return recipe_data
            ...
            if recipe_data and recipe_data.get('recipeIngredient'):
                ...
                recipe_data['sourceUrl'] = url
                recipe_data['extractedAt'] = time.strftime('%Y-%m-%d')
                recipe_data['extractionMethod'] = 'aggressive_html'
                return recipe_data
    ...
    def _extract_tastefully_simple(self, soup):
        ...
        # Current pages embed a serialized schema bundle in the BigCommerce
        # jsContext object rather than rendering ingredient list elements.
        for script in soup.find_all('script'):
            script_text = script.string or script.get_text() or ''
            marker = 'var jsContext = JSON.parse('
            if marker not in script_text:
                continue
            try:
                start = script_text.index(marker) + len(marker)
                serialized_context, _ = json.JSONDecoder().raw_decode(script_text[start:])
                context = json.loads(serialized_context)
                schema_bundle = json.loads(context.get('schemaJson', '{}'))
                schema = schema_bundle.get('Schema')
                if not isinstance(schema, dict):
                    continue

                displays = []
                for ingredient in schema_bundle.get('Ingredients', []):
                    if not isinstance(ingredient, dict):
                        continue
                    display = ingredient.get('IngredientDisplay') or ingredient.get('Display')
                    if display:
                        displays.append(BeautifulSoup(str(display), 'html.parser').get_text(' ', strip=True))
                if displays:
                    schema = dict(schema)
                    schema['recipeIngredient'] = displays

                parsed = self._parse_structured_recipe(schema)
                if parsed.get('recipeIngredient'):
                    print(f"    Parsed {len(parsed['recipeIngredient'])} embedded schema ingredients")
                    return parsed
            except (ValueError, TypeError, json.JSONDecodeError) as exc:
                print(f"    Embedded schema parsing failed: {exc}")
    ...
    def _parse_ingredient_string(self, ingredient_text):
        ...
        units = ['teaspoon', 'tsp', 'tablespoon', 'tbsp', 'cup', 'cups', 'ounce', 'oz', 'ounces', 
                ...
                'ml', 'milliliters', 'liter', 'liters', 'l', 'deciliter', 'deciliters', 'dl',
                'pinch', 'dash', 'can', 'package', 'pkg', 'bunch', 'bunches', 'clove',
